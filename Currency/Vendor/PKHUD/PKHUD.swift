...
open class PKHUD: NSObject {
    ...
    open func show(onView view: UIView? = nil) {
        DispatchQueue.main.async {
            let view: UIView = view ?? self.viewToPresentOn ?? UIApplication.shared.keyWindow!
            if  !view.subviews.contains(self.container) {
                view.addSubview(self.container)
                self.container.frame.origin = CGPoint.zero
                self.container.frame.size = view.frame.size
                self.container.autoresizingMask = [ .flexibleHeight, .flexibleWidth ]
                self.container.isHidden = true
            }
            if self.dimsBackground {
                self.container.showBackground(animated: true)
            }

            // If the grace time is set, postpone the HUD display
            if self.gracePeriod > 0.0 {
                let timer = Timer(timeInterval: self.gracePeriod, target: self, selector: #selector(PKHUD.handleGraceTimer(_:)), userInfo: nil, repeats: false)
                #if swift(>=4.2)
